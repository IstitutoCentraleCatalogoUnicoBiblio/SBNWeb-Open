...
import it.iccu.sbn.web.actionforms.gestionesemantica.classificazione.FondiClasseForm;
...
public class FondiClasseAction extends LookupDispatchAction {

	private static Log log = LogFactory.getLog(FondiClasseAction.class);
	...
	protected ActionForward unspecified(ActionMapping mapping, ActionForm form,
		...
		FondiClasseForm currentForm = (FondiClasseForm) form;
		...
		if (!currentForm.isSessione()) {
			...
			log.info("FondiClasseAction::unspecified");
			// devo inizializzare tramite le request.getAttribute(......)
			notazioneTrascinaDa = (String) request.getAttribute(NavigazioneSemantica.TRASCINA_CLASSE_PARTENZA);
			testoTrascinaDa = (String) request.getAttribute(NavigazioneSemantica.TRASCINA_TESTO_PARTENZA);
			notazioneTrascinaA = (String) request.getAttribute(NavigazioneSemantica.TRASCINA_CLASSE_ARRIVO);
			...
			ricSoggRicerca = (RicercaComuneVO) request.getAttribute(NavigazioneSemantica.PARAMETRI_RICERCA);
			...
			int i;
			for (i = 0; i < sinteticaTrascina.size(); i++) {
				ListaTitoliTrascinaVO lisTrasVO = new ListaTitoliTrascinaVO();
				lisTrasVO.setProgr(i + 1);
				lisTrasVO.setBid(((SinteticaTitoliView) sinteticaTrascina
						.get(i)).getBid());
				lisTrasVO
						.setIsbdELegami(((SinteticaTitoliView) sinteticaTrascina
								.get(i)).getDescrizioneLegami());
				lisTrasVO.setStato(((SinteticaTitoliView) sinteticaTrascina
						.get(i)).getLivelloAutorita());
			...
			currentForm.setOutputTrascina(ricSoggLista);
	...
	public ActionForward caricaBlocco(ActionMapping mapping, ActionForm form,
		...
		areaDatiPass.setRicercaPolo(currentForm.getRicercaComune().isPolo());
		UserVO utenteCollegato = Navigation.getInstance(request).getUtente();

		FactoryEJBDelegate factory = FactoryEJBDelegate.getInstance();


		AreaDatiPassaggioInterrogazioneTitoloReturnVO areaDatiPassReturn = factory.getGestioneBibliografica().getNextBloccoTitoli(
						areaDatiPass, utenteCollegato.getTicket());
		...
		for (i1 = 0; i1 < areaDatiPassReturn.getListaSintetica().size(); i1++) {
			ListaTitoliTrascinaVO lisTrasVO = new ListaTitoliTrascinaVO();
			lisTrasVO.setProgr(((SinteticaTitoliView) areaDatiPassReturn.getListaSintetica()
					.get(i1)).getProgressivo());
			lisTrasVO.setBid(((SinteticaTitoliView) areaDatiPassReturn.getListaSintetica()
					.get(i1)).getBid());
			lisTrasVO
					.setIsbdELegami(((SinteticaTitoliView) areaDatiPassReturn.getListaSintetica()
							.get(i1)).getDescrizioneLegami());
			lisTrasVO.setStato(((SinteticaTitoliView) areaDatiPassReturn.getListaSintetica()
					.get(i1)).getLivelloAutorita());
			lisTrasVO.setUtilizzato("SI");
	...
	public ActionForward ok(ActionMapping mapping, ActionForm form,
		...
		if (appoLista.size() > 0) {
			errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage(
					"errors.gestioneSemantica.okFus"));
			this.saveErrors(request, errors);
	...
	public ActionForward si(ActionMapping mapping, ActionForm form,
		...
		area.setIdElementoEliminato(currentForm.getNotazioneTrascinaDa());
		area.setIdElementoAccorpante(currentForm.getNotazioneTrascinaA());
		...
		try {
			...
			ritorno = factory.getGestioneSemantica().trascinaTitoliTraClassi(
					currentForm.getRicercaClasse().isPolo(), area,
					utenteCollegato.getTicket());

			log.info(ritorno);

			factory.getGestioneSemantica().cancellaClasse(
					((FondiClasseForm) form).getRicercaClasse()
									.isPolo(),
		...
		this.saveErrors(request, errors);
		currentForm.getRicercaClasse().setPolo(true);
		request.setAttribute(NavigazioneSemantica.OUTPUT_SINTETICA, currentForm.getOutputTrascina());

...
<%@page import="it.iccu.sbn.web.actionforms.amministrazionesistema.polo.ProfilazioneSemanticaForm;"%>
<html:xhtml />
<layout:page>
<div id="divForm">
<sbn:navform action="/amministrazionesistema/abilitazionePolo/profilazionePoloSemantica.do">
	<div id="divMessaggio">
		...
		<table align="center" border="0" width="100%">
			<tr>
				<td align="left" style="font-weight: bold; font-size: 15px" width="100%">
					<bean:message key="profilo.polo.parametri.sem.titolo" bundle="amministrazioneSistemaLabels"/>:
						<c:choose>
							<c:when test="${profilazionePoloSemanticaForm.tipologia eq 'CLA'}">
								<bean:message key="profilo.polo.parametri.classificazioni"	bundle="amministrazioneSistemaLabels" />
							</c:when>
							<c:when test="${profilazionePoloSemanticaForm.tipologia eq 'SOGG'}">
								<bean:message key="profilo.polo.parametri.soggetti"	bundle="amministrazioneSistemaLabels" />
							</c:when>
							<c:when test="${profilazionePoloSemanticaForm.tipologia eq 'THE'}">
			...
			<br/>
			 ...
			 <table class="sintetica" style="width: 600px">
				...
				<logic:iterate id="item" property="elencoParSemantica" name="profilazionePoloSemanticaForm" indexId="riga">
		 			<c:if test="${item.acceso eq 'TRUE'}">
				 		<sbn:rowcolor var="color" index="riga" />
							<tr bgcolor="${color}">
								<c:choose>
									<c:when test="${profilazionePoloSemanticaForm.nuovo eq 'TRUE'}">
				...
				<c:if test="${profilazionePoloSemanticaForm.nuovo eq 'TRUE'}">
						<tr bgcolor="${color}">
							<td  style="text-align: center">

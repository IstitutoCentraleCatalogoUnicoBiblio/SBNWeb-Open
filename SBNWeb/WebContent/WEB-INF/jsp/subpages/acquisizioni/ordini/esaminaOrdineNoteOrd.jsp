...
<c:when test="${formName eq 'esaminaOrdineForm'}">
	...
	<c:when test="${esaminaOrdineForm.disabilitaTutto}">
		...
		<tr>
          <td  class="etichetta"><bean:message  key="ordine.label.noteOrd" bundle="acquisizioniLabels" /></td>
          <td >
			<html:textarea styleId="testoNormale"  property="datiOrdine.noteOrdine"  rows="1" cols="70" readonly="${ordineAperto}"  ></html:textarea>

			<c:choose>
			<c:when test="${formName eq 'esaminaOrdineForm'}">
				<c:choose>
				<c:when test="${!esaminaOrdineForm.disabilitaTutto}">
					<sbn:tastiera limit="80" property="datiOrdine.noteOrdine" name="esaminaOrdineForm"></sbn:tastiera>
				</c:when>
				</c:choose>
			</c:when>
			<c:otherwise>
				...
				<c:when test="${formName eq 'esaminaOrdineModForm'}">
					...
					<c:when test="${!esaminaOrdineModForm.ordineApertoAbilitaInput &&  !esaminaOrdineModForm.disabilitaTutto}">
						<sbn:tastiera limit="80" property="datiOrdine.noteOrdine" name="esaminaOrdineModForm"></sbn:tastiera>
					</c:when>
					</c:choose>
				</c:when>
				</c:choose>

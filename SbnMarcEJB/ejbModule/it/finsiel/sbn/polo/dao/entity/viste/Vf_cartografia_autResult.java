 ...
 *
 */
public class Vf_cartografia_autResult extends it.finsiel.sbn.polo.dao.common.viste.Vf_cartografia_autCommonDao{

    public Vf_cartografia_autResult(Vf_cartografia_aut vf_cartografia_aut) throws InfrastructureException {
        super();
        this.valorizzaParametro(vf_cartografia_aut.leggiAllParametro());
    }
	/**
	 ...
	 */
	public List<Vf_cartografia_aut> selectPerCoordinate(HashMap opzioni)
			throws InfrastructureException {
		try {
			HashMap myOpzioni = (HashMap) opzioni.clone();
			Session session = this.getSession();
			this.beginTransaction();
			this.basicCriteria = session.createCriteria(Vf_cartografia_aut.class);
			Filter filter = session.enableFilter("VF_CARTOGRAFIA_AUT_selectPerCoordinate");
			...
			List<Vf_cartografia_aut> result = this.basicCriteria
					.list();
	...
	<statement nome="countPerCoordinate" tipo="count" id="04_taymer">
			<fisso>
				SELECT COUNT (distinct (bid) ) FROM vf_cartografia_aut
				WHERE
				tp_scala = XXXtipoScala
			</fisso>
			<!-- campi cartografici: sono canali di ricerca, ma non da utilizzarsi in mutua esclusione -->

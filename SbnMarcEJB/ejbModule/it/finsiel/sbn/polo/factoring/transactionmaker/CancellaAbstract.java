 ...
 */
public class CancellaAbstract extends CancellaFactoring{
	private String 			_idCancellazione;
	private Tb_abstract		abstractDaCancellare;
	private String				_codiceUtente;
	private TimestampHash		_timeHash = new TimestampHash();

	public CancellaAbstract(SBNMarc input_root_object) {
    ...
    private void executeCancella() throws IllegalArgumentException, InvocationTargetException, Exception {
		...
		abstractDaCancellare = abstractValida.validaPerCancella(user,_idCancellazione,_timeHash);
        if (abstractDaCancellare == null) {
            log.error("Errore nel factoring ricorsivo: oggetto tavola_response nullo");
            throw new EccezioneFactoring(201);
        }
        Abstract abs = new Abstract();
		abs.cancellaAbstract(abstractDaCancellare,_codiceUtente,_timeHash);

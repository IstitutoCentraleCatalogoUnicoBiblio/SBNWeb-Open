 ...
 * @version 3-mar-2003
 */
public class CreaTitoloUniforme extends CreaElementoAutFactoring {
    ...
    private TitoloUniformeType titUni;
    ...
    private final static int maxRighe =
        Integer.parseInt(
                ResourceLoader.getPropertyString("RIGHE_PER_BLOCCO_SBNMARC"));
     ...
     */
    public CreaTitoloUniforme(SBNMarc input_root_object) {
...
//        }
    ...
    protected void executeCrea()
        throws
            IllegalArgumentException, InvocationTargetException, InfrastructureException, Exception {
        //tipoForma=null;
        ...
        if (validaTitolo.validaPerCrea(getCdUtente(),ignoraID, timeHash,_cattura)) {
            ...
            if (ignoraID) {
                ...
                tb_titolo.setBID(
                    progress.getNextIdTitolo(tb_titolo.getTP_MATERIALE(),tb_titolo.getTP_RECORD_UNI()));
            } else
            ...
            if (datiEl.getNaturaTU() == null || !datiEl.getNaturaTU().equals("V")) { //si gestisce opera 231
                tb_titset_2.setUTE_INS(user);
                tb_titset_2.setUTE_VAR(user);
                tb_titset_2.setFL_CANC("N");
                tb_titset_2.setBID(tb_titolo.getBID());
                Titset2 titset2DB = new Titset2();
                titset2DB.inserisci(tb_titset_2);
            }
    ...
    private SBNMarc formattaOutput() throws EccezioneDB, EccezioneFactoring {
        ...
        datiResp.setTipoAuthority(SbnAuthority.TU);
	...
	public String getIdAttivitaSt() {
		if (_cattura) {// SE SONO IN CATTURA DEVO SOLO CONTROLLARE CHE SIA
			...
			return CodiciAttivita.getIstance().CREA_TITOLO_UNIFORME_1254;
		}
	}

/*******************************************************************************
 * Copyright (C) 2019 ICCU - Istituto Centrale per il Catalogo Unico
 * 
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published
 * by the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 * 
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 * 
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 ******************************************************************************/
package it.finsiel.sbn.polo.factoring.util;

import it.finsiel.sbn.polo.oggetti.StopList;
import it.finsiel.sbn.polo.orm.Ts_stop_list;

import java.util.HashMap;
import java.util.List;
import java.util.SortedSet;
import java.util.TreeSet;

import org.apache.log4j.Category;

/**
 * Classe ElencoArticoli.java
 * <p>
 *
 * </p>
 *
 * @author
 * @author
 *
 * @version 15-ott-02
 */
public class ElencoArticoli implements Elenco {
    /**
     * HashMap di stoplist nelle varie lingue:
     * contiene coppie di tipo (lingua,sortedSet).
     * La sortedSet (di tipo TreeSet) dovrebbe migliorare l'efficienza della ricerca.
     */
    private static HashMap elenchi = new HashMap();
    private static final ElencoArticoli instance = new ElencoArticoli();

    static Category log = Category.getInstance("iccu.box.ElencoArticoli");

    /**
     * Costruttore
     */
    private ElencoArticoli() {
    }

    /**
    * Inizializza la lista degli articoli, solo per le prove
    * @param nomeFile
     * @throws Exception
     * @throws Exception
    */
    public void init() throws Exception {
    	Decodificatore.instance.w.lock();
    	log.debug("Initializing ElencoArticoli");
        try {
			StopList stoplist = new StopList();
			elenchi = new HashMap();
			List<Ts_stop_list> v = stoplist.leggiArticoli();
			SortedSet stopList;
			Ts_stop_list sl;
			for (int i = 0; i < v.size(); i++) {
				sl = v.get(i);
				stopList = (SortedSet) elenchi.get(sl.getCD_LINGUA());
				if (stopList == null) {
					stopList = new TreeSet();
					elenchi.put(sl.getCD_LINGUA(), stopList);
				}
				stopList.add(sl.getPAROLA());
			}
		} finally {
			Decodificatore.instance.w.unlock();
		}
    }

    /**
     * Verifica se un elemento di una lingua è contenuto nella
     * relativa stop list.
     */
    public boolean contiene(String articolo, String lingua) {
    	Decodificatore.instance.r.lock();
        try {
			if (lingua == null)
				//return contiene(articolo);
				lingua = "ITA";
			SortedSet set = (SortedSet) elenchi.get(lingua.toUpperCase());
			if (set == null)
				return false;
			return set.contains(articolo.toUpperCase());
		} finally {
			Decodificatore.instance.r.unlock();
		}
    }

    /**
     * Verifica se un elemento di una lingua è contenuto nella
     * relativa stop list.
     */
    public boolean contiene(String articolo) {
        return contiene(articolo, null);
    }

	public static ElencoArticoli getInstance() {
		return instance;
	}

}

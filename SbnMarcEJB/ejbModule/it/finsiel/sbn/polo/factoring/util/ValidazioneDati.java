...
public class ValidazioneDati {
	...
	private static final String DEWEY_SYMBOL_REGEX = "^\\d{3}$|^\\d{3}\\.\\d+$|^\\d{3}\\.\\d+[a-zA-Z]{1}$";//"\\d{3}|\\d{3}\\.\\d+";
	...
	public static final boolean isT001Dewey(String id) {
		...
		return
			id.substring(0, 1).equalsIgnoreCase("D") &&
			strIsNumeric(id.substring(1, 3)) &&
			//almaviva5_20141114 test edizione ridotta
			(in(id.charAt(3), 'r', 'R') || !id.substring(3, 5).equals("  ") );
	}

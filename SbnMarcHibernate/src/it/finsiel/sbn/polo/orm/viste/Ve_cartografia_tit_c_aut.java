 ...
 */
public class Ve_cartografia_tit_c_aut extends V_cartografia {

	private static final long serialVersionUID = 3948288376445377179L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Ve_cartografia_tit_c_aut))
			return false;
		Ve_cartografia_tit_c_aut ve_cartografia_tit_c_aut = (Ve_cartografia_tit_c_aut)aObj;
		if (getBID_COLL() != null && !getBID_COLL().equals(ve_cartografia_tit_c_aut.getBID_COLL()))
			return false;
		if (getTP_LEGAME() != null && !getTP_LEGAME().equals(ve_cartografia_tit_c_aut.getTP_LEGAME()))
			return false;
		if (getBID() != null && !getBID().equals(ve_cartografia_tit_c_aut.getBID()))
			return false;
		if (getTP_RESPONSABILITA() != ve_cartografia_tit_c_aut.getTP_RESPONSABILITA())
			return false;
		if (getCD_RELAZIONE() != null && !getCD_RELAZIONE().equals(ve_cartografia_tit_c_aut.getCD_RELAZIONE()))
			return false;
		if (getVID() != null && !getVID().equals(ve_cartografia_tit_c_aut.getVID()))
			return false;
		return true;
	}

 ...
 */
public class Ve_grafica_sog_aut extends Tb_titolo implements Serializable {

	private static final long serialVersionUID = 1134059608868810075L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Ve_grafica_sog_aut))
			return false;
		Ve_grafica_sog_aut ve_grafica_sog_aut = (Ve_grafica_sog_aut)aObj;
		if (getCID() != null && !getCID().equals(ve_grafica_sog_aut.getCID()))
			return false;
		if (getBID() != null && !getBID().equals(ve_grafica_sog_aut.getBID()))
			return false;
		if (getTP_RESPONSABILITA() != ve_grafica_sog_aut.getTP_RESPONSABILITA())
			return false;
		if (getCD_RELAZIONE() != null && !getCD_RELAZIONE().equals(ve_grafica_sog_aut.getCD_RELAZIONE()))
			return false;
		if (getVID() != null && !getVID().equals(ve_grafica_sog_aut.getVID()))
			return false;
		return true;
	}

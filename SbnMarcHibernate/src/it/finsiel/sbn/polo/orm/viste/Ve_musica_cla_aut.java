 ...
 */
public class Ve_musica_cla_aut extends Tb_titolo implements Serializable {

	private static final long serialVersionUID = -7487443512789112449L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Ve_musica_cla_aut))
			return false;
		Ve_musica_cla_aut ve_musica_cla_aut = (Ve_musica_cla_aut)aObj;
		if (getCD_SISTEMA() != ve_musica_cla_aut.getCD_SISTEMA())
			return false;
		if (getCD_EDIZIONE() != ve_musica_cla_aut.getCD_EDIZIONE())
			return false;
		if (getCLASSE() != null && !getCLASSE().equals(ve_musica_cla_aut.getCLASSE()))
			return false;
		if (getBID() != null && !getBID().equals(ve_musica_cla_aut.getBID()))
			return false;
		if (getTP_RESPONSABILITA() != ve_musica_cla_aut.getTP_RESPONSABILITA())
			return false;
		if (getCD_RELAZIONE() != null && !getCD_RELAZIONE().equals(ve_musica_cla_aut.getCD_RELAZIONE()))
			return false;
		if (getVID() != null && !getVID().equals(ve_musica_cla_aut.getVID()))
			return false;
		return true;
	}

 ...
 */
public class Ve_musica_luo_aut extends Tb_titolo implements Serializable {

	private static final long serialVersionUID = 8909861573436226809L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Ve_musica_luo_aut))
			return false;
		Ve_musica_luo_aut ve_musica_luo_aut = (Ve_musica_luo_aut)aObj;
		if (getLID() != null && !getLID().equals(ve_musica_luo_aut.getLID()))
			return false;
		if (getTP_LUOGO() != ve_musica_luo_aut.getTP_LUOGO())
			return false;
		if (getBID() != null && !getBID().equals(ve_musica_luo_aut.getBID()))
			return false;
		if (getTP_RESPONSABILITA() != ve_musica_luo_aut.getTP_RESPONSABILITA())
			return false;
		if (getCD_RELAZIONE() != null && !getCD_RELAZIONE().equals(ve_musica_luo_aut.getCD_RELAZIONE()))
			return false;
		if (getVID() != null && !getVID().equals(ve_musica_luo_aut.getVID()))
			return false;
		return true;
	}

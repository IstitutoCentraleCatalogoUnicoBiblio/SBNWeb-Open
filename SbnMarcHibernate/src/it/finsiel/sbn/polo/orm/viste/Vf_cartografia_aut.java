 ...
 */
public class Vf_cartografia_aut extends V_cartografia {

	private static final long serialVersionUID = 5743515446266954593L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Vf_cartografia_aut))
			return false;
		Vf_cartografia_aut vf_cartografia_aut = (Vf_cartografia_aut)aObj;
		if (getBID() != null && !getBID().equals(vf_cartografia_aut.getBID()))
			return false;
		if (getTP_RESPONSABILITA() != vf_cartografia_aut.getTP_RESPONSABILITA())
			return false;
		if (getCD_RELAZIONE() != null && !getCD_RELAZIONE().equals(vf_cartografia_aut.getCD_RELAZIONE()))
			return false;
		if (getVID() != null && !getVID().equals(vf_cartografia_aut.getVID()))
			return false;
		return true;
	}

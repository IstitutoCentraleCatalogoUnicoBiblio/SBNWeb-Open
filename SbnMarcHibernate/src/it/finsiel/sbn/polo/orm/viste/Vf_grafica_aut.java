 ...
 */
public class Vf_grafica_aut extends Tb_titolo implements Serializable {

	private static final long serialVersionUID = -6944146129743082866L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Vf_grafica_aut))
			return false;
		Vf_grafica_aut vf_grafica_aut = (Vf_grafica_aut)aObj;
		if (getBID() != null && !getBID().equals(vf_grafica_aut.getBID()))
			return false;
		if (getTP_RESPONSABILITA() != vf_grafica_aut.getTP_RESPONSABILITA())
			return false;
		if (getCD_RELAZIONE() != null && !getCD_RELAZIONE().equals(vf_grafica_aut.getCD_RELAZIONE()))
			return false;
		if (getVID() != null && !getVID().equals(vf_grafica_aut.getVID()))
			return false;
		return true;
	}

 ...
 */
public class Vf_musica_aut extends Tb_titolo implements Serializable {

	private static final long serialVersionUID = 8513232129777792658L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Vf_musica_aut))
			return false;
		Vf_musica_aut vf_musica_aut = (Vf_musica_aut)aObj;
		if (getBID() != null && !getBID().equals(vf_musica_aut.getBID()))
			return false;
		if (getTP_RESPONSABILITA() != vf_musica_aut.getTP_RESPONSABILITA())
			return false;
		if (getCD_RELAZIONE() != null && !getCD_RELAZIONE().equals(vf_musica_aut.getCD_RELAZIONE()))
			return false;
		if (getVID() != null && !getVID().equals(vf_musica_aut.getVID()))
			return false;
		return true;
	}

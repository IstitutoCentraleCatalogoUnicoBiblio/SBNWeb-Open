 ...
 */
public class Vl_biblioteca_sog extends Tbf_biblioteca_in_polo implements Serializable {

	private static final long serialVersionUID = -4729494592391006355L;

	public boolean equals(Object aObj) {
		if (aObj == null)
			return false;
		if (!(aObj instanceof Vl_biblioteca_sog))
			return false;
		Vl_biblioteca_sog vl_biblioteca_sog = (Vl_biblioteca_sog)aObj;
		if (getBID() != null && !getBID().equals(vl_biblioteca_sog.getBID()))
			return false;
		if (getCod_polo() != null && !getCod_polo().equals(vl_biblioteca_sog.getCod_polo()))
			return false;
		if (getCd_biblioteca() != null && !getCd_biblioteca().equals(vl_biblioteca_sog.getCd_biblioteca()))
			return false;
		if (getCID() != null && !getCID().equals(vl_biblioteca_sog.getCID()))
			return false;
		if (getFL_ALLINEA_REP() != vl_biblioteca_sog.getFL_ALLINEA_REP())
			return false;
		return true;
	}
	...
	public int hashCode() {
		...
		hashcode = hashcode + getFL_ALLINEA_REP().hashCode();
		return hashcode;
	}
	...
	public String toString() {
		return String.valueOf(getBID() + " " + getCod_polo() + " " + getCd_biblioteca() + " " + getCID() + " " + getFL_ALLINEA_REP());
	}

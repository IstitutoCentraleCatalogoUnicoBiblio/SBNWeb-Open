 ...
 *
 * @ejb.bean name="SBNBollettino"
 * 			 acknowledge-mode="Auto-acknowledge"
 *           destination-type="javax.jms.Queue"
 *           transaction-type="Container"
 *           destination-jndi-name="SBNBollettino"
 ...
 *
 */
public class SBNStampeBollettinoBean extends SBNStampeBase implements MessageDrivenBean, MessageListener {
	...
	static Logger log = Logger.getLogger(SBNStampeBollettinoBean.class);
	...
	throws Exception {
		...
		List listaBollettino = new ArrayList();
...
//		byte[] streamLogStampa;
		...
		try {
			...
			String textListEm="ATTENZIONE: bollettino ricercato inesistente, cambiare i parametri di ricerca";
			elaborazioniDifferiteOutputVo.setStato(ConstantsJMS.STATO_OK);

			logger.info("path fileJrxml per la stampa bollettino"+fileJrxml);
			System.out.print(""+fileJrxml);

			StampaBollettinoVO bollettino = null;
			...
			StampaBollettinoVO stampaBollettinoVO = (StampaBollettinoVO)parametri.get(0);
			stampaBollettinoVO.setUser(utente);
			stampaBollettinoVO.setCodAttivita(codAtt);
			stampaBollettinoVO.setIdBatch(idMess);

				bollettino = this.getInventario().getBollettino(stampaBollettinoVO, ticket);
				if (bollettino == null || bollettino.getLista().size() == 0 ){
					elaborazioniDifferiteOutputVo.setStato(ConstantsJMS.STATO_OK);
					nameFile = utente +"_log_bollettino_"+idMess+ "." + "txt";
			...
			try{
				...
				StampaBollettinoDettaglioVO elemListaVO;
				StampaBollettinoDettaglioVO recOut = new StampaBollettinoDettaglioVO(FakeParamRichiestaElaborazioneDifferitaVO.FAKE_INSTANCE);
				recOut.setCodBib(stampaBollettinoVO.getCodBib());//
				recOut.setDescrBib(bollettino.getDescrBib());//
				recOut.setDataDiElaborazione(DateUtil.formattaData(DaoManager.now().getTime()));
				recOut.setCodPolo(stampaBollettinoVO.getCodPolo());
				recOut.setCodBib(stampaBollettinoVO.getCodBib());
				recOut.setCodAttivita(stampaBollettinoVO.getCodAttivita());
				recOut.setUser(stampaBollettinoVO.getUser());
				recOut.setDataDa(stampaBollettinoVO.getDataDa());
				recOut.setDataA(stampaBollettinoVO.getDataA());
				recOut.setOrdinaPer(bollettino.getOrdinaPer());
				if (stampaBollettinoVO.isNuoviEsemplari()){
					recOut.setTipoLista("Tutti i nuovi Esemplari");
				}else{
					recOut.setTipoLista("Nuovi titoli entrati in biblioteca");
				}
				for(int indice=0; indice < bollettino.getLista().size(); indice++){
					elemListaVO = (StampaBollettinoDettaglioVO)bollettino.getLista().get(indice);
					recOut.getLista().add(elemListaVO);
				}
				listaBollettino.add(recOut);

				boolean listaEsistente = false;
				if(bollettino.getLista().size()>0){
					//genero la stampa
					...
					if(streamRichiestaStampa !=null){
						try {
							nameFile =  stampaVO.getFirmaBatch() +"_bollettino_"+idMess+ "." + tipoStampa;
						...
						property = stampaBollettinoVO.getCodBib();
						if (property != null && property.length() > 0) {
							hm.put("Codice biblioteca", property);
						}

						property = stampaBollettinoVO.getCodPolo();
						if (property != null && property.length() > 0) {

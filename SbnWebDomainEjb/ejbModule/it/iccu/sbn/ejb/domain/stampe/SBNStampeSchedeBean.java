 ...
 *
 * @ejb.bean name="SBNSchede"
 * 			 acknowledge-mode="Auto-acknowledge"
 *           destination-type="javax.jms.Queue"
 *           transaction-type="Container"
 *           destination-jndi-name="SBNSchede"
 ...
 *
 */
public class SBNStampeSchedeBean extends SBNStampeBase  {
	/**
	 *
	 */
	...
	private static Logger log = Logger.getLogger(SBNStampeSchedeBean.class);
						...
						if(datiStampaSchede.getListaPerTitoloSchedeVO().size()>0){
							...
							if(streamRichiestaStampa !=null){
								try {
									...
									// Gestione dei nuovi criteri estrazione/ordinamento Stampa Cataloghi
									if (datiStampaSchede.isRichListaPerTitolo()) {
										nameFile = utente +"_catalografico_PerTitolo_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									} else if (datiStampaSchede.isRichListaPerAutore()) {
										nameFile = utente +"_catalografico_PerAutore_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									} else if (datiStampaSchede.isRichListaPerClassificazione()) {
										nameFile = utente +"_catalografico_PerClassificazione_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									} else if (datiStampaSchede.isRichListaPerEditore()) {
										nameFile = utente +"_catalografico_PerEditore_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									} else if (datiStampaSchede.isRichListaPerPossessore()) {
										nameFile = utente +"_catalografico_PerPossessore_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									} else if (datiStampaSchede.isRichListaPerSoggetto()) {
										nameFile = utente +"_catalografico_PerSoggetto_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									} else if (datiStampaSchede.isRichListaPerTopografico()) {
										nameFile = utente +"_catalografico_PerTopografico_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									} else {
										nameFile = utente +"_catalografico_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
									}
...
//						fine blocco ListaPerTitolo
			...
			elaborazioniDifferiteOutputVo.setStato(ConstantsJMS.STATO_ERROR);
			nameFile = utente +"_log_schede_"+idMess+ "." + "txt";

 ...
 *
 * @ejb.bean name="SBNSuggerimentiBibliotecario"
 * 			 acknowledge-mode="Auto-acknowledge"
 *           destination-type="javax.jms.Queue"
 *           transaction-type="Container"
 *           destination-jndi-name="SBNSuggerimentiBibliotecario"
 ...
 *
 */
public class SBNStampeSuggerimentiBibliotecarioBean extends SBNStampeBase implements MessageDrivenBean, MessageListener {
	/**
	 *
	 */
	...
	private static Logger log = Logger.getLogger(SBNStampeSuggerimentiBibliotecarioBean.class.getName());
	...
	throws ResourceNotFoundException, ApplicationException, ValidationException, DataException	{
		...
		try {
			...
			if (stampaVO instanceof StampaOnLineVO) {
				...
				log.info("path fileJrxml per la stampa suggerimenti bibliotecario: "+stampaVO.getTemplate());
				...
				try {
					...
					if(datiDaStampare.size()>0){
						...
						if(streamRichiestaStampa !=null){
							try {
								//nameFile = utente +"_suggerimenti_bibliotecario_"+idMess+ "." + stampaVO.getTipoStampa().toLowerCase();
			...
			elaborazioniDifferiteOutputVo.setStato(ConstantsJMS.STATO_ERROR);
			nameFile = utente +"_log_suggerimenti_bibliotecario_"+idMess+ "." + "txt";
...
//					log.info("Terminata l'elaborazione del messaggio in SBNStampeSuggerimentiBibliotecarioBean");

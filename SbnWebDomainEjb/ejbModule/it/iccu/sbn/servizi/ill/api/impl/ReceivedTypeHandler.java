...
public class ReceivedTypeHandler extends AvanzamentoRichiestaILLBaseHandler {

	private static final long serialVersionUID = 924247275220806751L;

	private ReceivedType received;

	protected ReceivedTypeHandler(String ticket, ILLAPDU input) {
		super(ticket, input);
		received = input.getReceived();
	}

	public void execute() throws ApplicationException, ValidationException {
		try {
			//1. ricerca richiesta ill
			DatiRichiestaILLVO dati_richiesta_ill = getDatiRichiestaIll(received);
			//memorizzazione stato richiesta
			String oldState = dati_richiesta_ill.getCurrentState();

			//nuovo stato
			String newState = StatoIterRichiesta.F114_ARRIVO_MATERIALE.getISOCode();

			//prepara messaggio
			MessaggioVO msg = preparaMessaggio(StatoIterRichiesta.of(newState), dati_richiesta_ill.getRuolo(),
					received.getRequesterNote());
	...
	public String getXMLName() {
		return formattaNomeXML(received.getTransactionId(), StatoIterRichiesta.F114_ARRIVO_MATERIALE,
				received.getResponderId());
	}

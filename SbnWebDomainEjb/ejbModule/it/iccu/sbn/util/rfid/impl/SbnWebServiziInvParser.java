...
public class SbnWebServiziInvParser implements InventarioRFIDParsingStrategy {

	private static final int MAX_LEN = String.valueOf(DocumentoFisicoCostant.MAX_PROGRESSIVO_INVENTARIO).length();
	...
	public InventarioVO parse(final String rfid) throws Exception {
		...
		String codInv = ValidazioneDati.rtrim(rfid.substring(3, len));
		if (!ValidazioneDati.strIsNumeric(codInv))
			return null;

		if (codInv.length() > MAX_LEN)
			return null;

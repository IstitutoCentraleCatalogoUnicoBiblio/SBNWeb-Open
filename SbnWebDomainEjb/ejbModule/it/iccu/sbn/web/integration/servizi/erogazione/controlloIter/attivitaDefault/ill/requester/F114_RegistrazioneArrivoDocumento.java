...
public class F114_RegistrazioneArrivoDocumento extends AbstractAttivitaCheckerILL {

	public F114_RegistrazioneArrivoDocumento() throws RemoteException, NamingException, CreateException {
		super();
	}

	@Override
	public ControlloAttivitaServizioResult check(DatiControlloVO dati) throws Exception {
		dati.setControlloEseguito(StatoIterRichiesta.F114_ARRIVO_MATERIALE);
		MovimentoVO mov = dati.getMovimento();
		Timestamp now = DaoManager.now();
		mov.setDataInizioEff(now);
		DatiRichiestaILLVO datiILL = mov.getDatiILL();
		//se è riproduzione la richiesta ill è conclusa
		if (ILLServiceType.valueOf(datiILL.getServizio()) == ILLServiceType.RI)
			datiILL.setDataFine(now);

		//prepara messaggio
		MessaggioVO msg = dati.getUltimoMessaggio();
		msg = msg != null ? msg : new MessaggioVO(datiILL);
	...
	public ControlloAttivitaServizioResult post(DatiControlloVO dati) throws Exception {
		MessaggioVO msg = dati.getUltimoMessaggio();
		try {
			ILLAPDU response = ILLRequestBuilder.arrivoMateriale(dati.getMovimento().getDatiILL(), msg);
			return checkResponse(response, dati.setControlloEseguito(StatoIterRichiesta.F114_ARRIVO_MATERIALE));

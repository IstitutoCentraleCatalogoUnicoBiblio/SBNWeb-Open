...
public class F116B_MaterialeDanneggiato extends AbstractAttivitaCheckerILL {

	public F116B_MaterialeDanneggiato() throws RemoteException, NamingException, CreateException {
		super();
	}

	@Override
	public ControlloAttivitaServizioResult check(DatiControlloVO dati) throws Exception {
		dati.setControlloEseguito(StatoIterRichiesta.F116B_MATERIALE_DANNEGGIATO);
		...
		msg.setStato(StatoIterRichiesta.F116B_MATERIALE_DANNEGGIATO.getISOCode());
	...
	public ControlloAttivitaServizioResult post(DatiControlloVO dati) throws Exception {
		...
		try {
			ILLAPDU response = ILLRequestBuilder.materialeDanneggiato(dati.getMovimento().getDatiILL(), msg);
			return checkResponse(response, dati.setControlloEseguito(StatoIterRichiesta.F116B_MATERIALE_DANNEGGIATO));

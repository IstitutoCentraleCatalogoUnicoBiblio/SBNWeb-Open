...
//	CString tbcSezioneCollocazioneFilename;
	...
	long elementsTbParola;	// Numero di autori
	...
	long elementsTsLinkMultim;	// Numero di autori
	...
	long elementsTbNota;	// Numero di autori
	char *offsetBufferTbNotaPtr;
	TbNota *tbNota;

	// Authority Titolo
	// ------------------------
	//CString tbTitoloFilename;
	CFile* tbTitoloIn;
	...
	//CString tbAutoreOffsetFilename;
	...
	long elementsTbcNotaInv;	// Numero di classi
	...
	int  trKeyPlusOffsetPlusLfLength;	// Lungezza del campo Chiave/Offset per le chiaci fatte da bid+bid, bid+vid, ecc.
...
//	tree<std::string> *creaReticoloAuthorityAutore(char * id);
	...
	void creaRecordUnimarcAuthority(char *id);
